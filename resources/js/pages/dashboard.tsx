...
import { useCompany }        from '@/hooks/use-company-storage'; // dashboard.tsx
    ...
    const inner = (
        <div
            className={cn(
                ...
                effectiveHref && 'cursor-pointer hover:-translate-y-0.5 hover:shadow-md',
...
export default function Dashboard({
    ...
    // ── Resolves: Inertia props → localStorage → cookie ──────────────────
    const company = useCompany();

    const breadcrumbs: BreadcrumbItem[] = [
        { title: 'Dashboard', href: '/dashboard' },
        ...(company ? [{ title: company.name, href: '/dashboard' }] : []),
    ];
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={company ? `${company.name} Dashboard` : 'Dashboard'} />
                ...
                <div className="flex flex-wrap items-start justify-between gap-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                        {canView('Submissions') && stats.submissions_new > 0 && (
                            <Link
                                href="/submissions"
                                className="flex items-center gap-2 rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-xs font-medium text-blue-700 transition-colors hover:bg-blue-100"
                            >
                        ...
                        {canView('Testimonials') && stats.testimonials_pending > 0 && (
                            <Link
                                href="/testimonials"
                                className="flex items-center gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-medium text-amber-700 transition-colors hover:bg-amber-100"
                            >
                    ...
                    {canView('Analytics') ? (
                        <div className="xl:col-span-2 rounded-2xl border bg-card p-6 shadow-sm">
                            <div className="mb-4 flex items-center justify-between">
                                ...
                                <Link href="/analytics/page-views" className="flex items-center gap-1 text-xs text-muted-foreground hover:text-primary">
                        ...
                        <div className="flex items-center justify-center rounded-2xl border bg-muted/30 p-6 shadow-sm xl:col-span-2">
                    ...
                    {canView('Workflow') && (
                        <div className="rounded-2xl border bg-card p-6 shadow-sm">
                            <div className="mb-4 flex items-center justify-between">
                                ...
                                <Link href="/workflow" className="flex items-center gap-1 text-xs text-muted-foreground hover:text-primary">
                    ...
                    {canView('Blog') && (
                        <div className="xl:col-span-2 rounded-2xl border bg-card p-6 shadow-sm">
                            <div className="mb-4 flex items-center justify-between">
                                ...
                                {canCreate('Blog') && (
                                    <Link href="/blog/create" className="flex items-center gap-1 text-xs text-primary hover:underline">
                                    ...
                                    {recentPosts.map((post) => (
                                        <div key={post.id} className="flex items-center gap-3 py-3">
                                            <div className="min-w-0 flex-1">
                                                <a href={post.edit_url} className="block truncate text-sm font-medium transition-colors hover:text-primary">
                                                    {post.title}
                                                </a>
                                                <div className="mt-0.5 flex items-center gap-2 text-xs text-muted-foreground">
                                                    {post.author && (
                                                        <span className="flex items-center gap-1">
                                                            <PenLine className="h-3 w-3" /> {post.author}
                                                        </span>
                                                    )}
                                                    <span className="flex items-center gap-1">
                                                        <Clock className="h-3 w-3" /> {post.created_at}
                                                    </span>
                                                </div>
                                            </div>
                        ...
                        {canView('Media') && (
                            <div className="rounded-2xl border bg-card p-5 shadow-sm">
                                ...
                                <p className="mt-0.5 text-xs text-muted-foreground">
                                    {storageStats.total_files} files total
                                </p>
                    ...
                    {canView('Submissions') && (
                        <div className="xl:col-span-2 rounded-2xl border bg-card p-6 shadow-sm">
                            <div className="mb-4 flex items-center justify-between">
                                ...
                                <Link href="/submissions" className="flex items-center gap-1 text-xs text-muted-foreground hover:text-primary">
